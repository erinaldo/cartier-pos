...
namespace ETLPOS
{
    public partial class frmSales : BaseForm
    {
        ...
        private void loaddiscount()
        {
            ...
            oResult = oMembershipBO.ReadAllByDateAndIsActive(currentBranch.CompBrn_Branch, txtDiscountMember.Text.Trim());
            if (oResult.IsSuccess)
            {
        ...
        {
            if (txtNetPay.Text.Trim() != "0")
            {
                loaddiscount();
            }
            else
            {
                MessageBox.Show("Sales Amount Is empty");
