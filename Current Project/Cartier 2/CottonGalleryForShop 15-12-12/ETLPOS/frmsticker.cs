...
namespace ETLPOS
{
    public partial class frmsticker : BaseForm
    {
        ...
        {
            AutoComplete(cmbItemName, e, false);
        }
        ...
        private void GetItemNameByText()
        {
            cmbItemName.Items.Clear();
            var items = oItemList.Where(t => t.Item_ItemName.ToLower().Contains(txtArticle.Text.ToLower()));
            foreach (CItemSales item in items)
            {
